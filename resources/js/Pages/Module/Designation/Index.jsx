...
function Index() {
    ...
    function editGroupCompany(result) {
        router.get("/admin/designation/edit/" + result.id);
    }
    function deleteGroupCompany(result) {
        router.get("/admin/designation/delete/" + result.id);
    }
    function statusChanged(result) {
        router.get("/admin/designation/status/" + result.id);
    }
    return (
        <>
            ...
            {
                permissions.includes('designation-view') || permissions.includes('super-admin') ? (
                    <div className="panel mt-6">
                        <div className="flex md:items-center flex-col sm:flex-row justify-between mb-5 gap-5">
                            ...
                            <div className="flex md:items-center gap-5">
                                ...
                                {
                                    permissions.includes('designation-create') || permissions.includes('super-admin') ? (
                                        <Link
                                            href={`${base_url}/admin/designation/create`}
                                            method="get"
                                    ...
                                    },
                                    {
                                        ...
                                        render: (result) => (
                                            <div className="flex items-center w-max mx-auto gap-2">
                                                {
                                                    permissions.includes('designation-edit') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/designation/edit/`+result.id} method="get" className="btn btn-sm btn-outline-primary">
                                                            Edit
                                                        </Link>
                                                    ) : null
                                                }
                                                {
                                                    permissions.includes('designation-delete') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/designation/delete/`+result.id} method="get" className="btn btn-sm btn-outline-danger">
                                                            Delete
                                                        </Link>
                                                    ) : null
                                                }
...
Index.layout = (page) => (
    <MainLayout children={page} title="HR || Designation" />
);
