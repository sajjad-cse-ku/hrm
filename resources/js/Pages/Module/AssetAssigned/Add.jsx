...
function Add() {
    const {  flash, asset_type, users,asset  } = usePage().props;
    ...
    const assetId = asset.map((item) => ({
        value: item.id,
        label: item?.product_name? `${item?.product_name} - ${item?.office_tag_number}` : '',
    }));
    ...
    function handleSubmit(e) {
        e.preventDefault();
        router.post("/admin/asset-assigned/store", values);
        setValues({
            employee_id: "",
            asset_type_id: "",
            asset_id:"",
...
Add.layout = (page) => (
    <MainLayout children={page} title="HR || Add Asset Assigned" />
);
