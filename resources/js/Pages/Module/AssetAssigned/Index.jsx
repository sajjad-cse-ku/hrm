...
function Index() {
    ...
    const [initialRecords2, setInitialRecords2] = useState(result, "location");
    ...
    useEffect(() => {
        setInitialRecords2(() => {
            return result.filter((item) => {
                return (
                    item.location && item.location.toLowerCase().includes(search2.toLowerCase())
                );
            });
        });
    }, [search2]);
    ...
    function statusChanged(result) {
        router.get("/admin/asset-assigned/status/" + result.id);
    }
                    ...
                    <div className="panel mt-6">
                        <div className="flex md:items-center flex-col sm:flex-row justify-between mb-5 gap-5">
                            ...
                            <div className="flex md:items-center gap-5">
                                ...
                                {/*    permissions.includes('super-admin') ? (*/}
                                        <Link
                                            href={`${base_url}/admin/asset-assigned/create`}
                                            method="get"
                                    ...
                                    },
                                    {
                                        ...
                                        render: (result) => (
                                            <div className="flex items-center w-max mx-auto gap-2">
                                                {/*{*/}
                                                {/*    permissions.includes('super-admin') ? (*/}
                                                        <Link href={`${base_url}/admin/asset-assigned/edit/`+result.id} method="get" className="btn btn-sm btn-outline-primary">
                                                            Edit
                                                        </Link>
                                                {/*    ) : null*/}
                                                {/*}*/}
                                                {/*{*/}
                                                {/*    permissions.includes('super-admin') ? (*/}
                                                        <Link href={`${base_url}/admin/asset-assigned/delete/`+result.id} method="get" className="btn btn-sm btn-outline-danger">
                                                            Delete
                                                        </Link>
                                                {/*    ) : null*/}
                                                {/*}*/}
...
Index.layout = (page) => (
    <MainLayout children={page} title="HR || Asset Assigned List" />
);
