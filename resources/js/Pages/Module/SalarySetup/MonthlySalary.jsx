...
function MonthlySalary() {
    const { base_url, flash, salaries } = usePage().props;
    console.log(salaries)
    const PAGE_SIZES = [10, 20, 30, 50, 100];
    const [page2, setPage2] = useState(1);
    const [pageSize2, setPageSize2] = useState(PAGE_SIZES[0]);
    const [initialRecords2, setInitialRecords2] = useState(salaries, "title");
    ...
    useEffect(() => {
        setInitialRecords2(() => {
            return salaries.filter((item) => {
                return (
                    (item?.user?.first_name && item?.user?.last_name.toLowerCase().includes(search2.toLowerCase()))
                );
            });
        });
    }, [search2]);
                            ...
                            },
                            {
                                ...
                                render: (salaries) => (
                                    <div className="flex items-center w-max mx-auto gap-2">
                                        <Link href={`${base_url}/admin/edit-monthly-salary/` + salaries.id} method="get"
...
MonthlySalary.layout = (page) => (
    <MainLayout children={page} title="HR || Monthly Salary"/>
);
