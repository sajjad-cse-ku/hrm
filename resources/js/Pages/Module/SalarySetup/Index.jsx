...
function Index() {
    ...
    const [initialRecords2, setInitialRecords2] = useState(result, "basic");
    ...
    useEffect(() => {
        setInitialRecords2(() => {
            return result.filter((item) => {
                return (
                    item.basic.toLowerCase().includes(search2.toLowerCase())
                );
            });
        });
    }, [search2]);
    ...
    function statusChanged(result) {
        router.get("/admin/salary-setups/status/" + result.id);
    }
    ...
    return (
        <>
            ...
            {
                permissions.includes('salary-setups-view') || permissions.includes('super-admin') ? (
                            ...
                            <DataTable
                                ...
                                columns={[
                                    ...
                                    },
                                    {
                                        ...
                                        render: ({ user }) => (
                                            <div className="flex items-center w-max">
                                                <div>{user?.first_name} {user?.last_name}-{user?.id}</div>
                                            </div>
                                        ),
                                    },
                                    {
                                    ...
                                    },
                                    {
                                        ...
                                        render: (result) => (
                                            <div className="flex items-center w-max mx-auto gap-2">
                                                {
                                                    permissions.includes('salary-setups-edit') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/salary-setups/edit/`+result.id} method="get" className="btn btn-sm btn-outline-primary">
                                                            Edit
                                                        </Link>
                                                    ) : null
                                                }
                                                {
                                                    permissions.includes('salary-setups-delete') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/salary-setups/delete/`+result.id} method="get" className="btn btn-sm btn-outline-danger">
                                                            Delete
                                                        </Link>
                                                    ) : null
                                                }
