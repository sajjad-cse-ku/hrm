...
function Index() {
    const {flash , weeks ,users,shifts,locations, permissions ,roster_status, } = usePage().props;
    ...
    const individualUserData = (userId) => {
        const userToSubmit = users.find((user) => user.id === userId);
        const locationObj = { ...locationValue[userId] };
        const dataToSubmit = {
            user_id: userToSubmit.id,
            location: locationObj,
            roster: rosterState[userId],
        };
        router.post("/admin/single-roster", dataToSubmit);
    };

    function onSubmit(data) {
        router.post("/admin/all-roster", data);
    }
    return (
        <>
                        ...
                        <div className="space-y-8 pt-5">
                            <div className="grid grid-cols-1 lg:grid-cols-1">
                                <div className="panel" id="line">
                                    {
                                        permissions.includes('roster-view') || permissions.includes('super-admin')  ? (
                                            <div className="mb-5">
                                                {
                                                    permissions.includes('roster-insert') || permissions.includes('super-admin')  ? (
                                                        <Tab.Group>
                                                                                                ...
                                                                                                <tr key={idx}>
                                                                                                        ...
                                                                                                        <td key={dayOfWeek}>
                                                                                                            <div>
                                                                                                                <select
                                                                                                                    ...
                                                                                                                    {...register(`[${user.id}].roster[${dayOfWeek}-${weeks[weekNumber][dayOfWeek]}]`)}
                                                                                                                    value={rosterState[user.id][weekNumber][weeks[weekNumber][dayOfWeek]]}
                                                                                                                    onChange={(e) =>
                                                                                                                        handleSelectChange(
                                                                                                                            user.id,
                                                                                                                            weekNumber,
                                                                                                    ...
                                                                                                    <td>
                                                                                                        <div>
                                                                                                            <select
                                                                                                                className="form-select text-white-dark"
                                                                                                                {...register(`[${user.id}].location[weak-${weekNumber}]`)}
                                                                                                                value={locationValue[user.id]?.[`weak-${weekNumber}`] || ''} // Set the value for the selected week
                                                                                                                onChange={(e) => handleLocationChange(user.id, weekNumber, e.target.value)} // Pass weekNumber to the handler
                                                                                                            >
                                                                                                                <option value="">Select....</option>
                                                                                                                {locations.map((item) => (
...
Index.layout = (page) => (
    <MainLayout children={page} title="HR || Roster Entry" />
);
