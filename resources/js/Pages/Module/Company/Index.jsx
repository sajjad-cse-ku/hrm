...
function Index() {
        ...
        setInitialRecords2(() => {
            return result.filter((item) => {
                return (
                    ...
                    (item.email && item.email.includes(search2)) ||
                    (item.address && item.address.toLowerCase().includes(search2.toLowerCase())) ||
                    (item?.groupcompany?.name && item?.groupcompany?.name.toLowerCase().includes(search2.toLowerCase())) ||
                    (item.city && item.city.toString().toLowerCase().includes(search2.toLowerCase()))
                );
            });
        });
    }, [search2]);
    ...
    function editGroupCompany(result) {
        router.get("/admin/companies/edit/" + result.id);
    }
    function deleteGroupCompany(result) {
        router.get("/admin/companies/delete/" + result.id);
    }
    function statusChanged(result) {
        router.get("/admin/companies/status/" + result.id);
    }
    return (
        <>
            ...
            {
                permissions.includes('company-view') || permissions.includes('super-admin') ? (
                    <div className="panel mt-6">
                        <div className="flex md:items-center flex-col sm:flex-row justify-between mb-5 gap-5">
                            ...
                            <div className="flex md:items-center gap-5">
                                ...
                                {
                                    permissions.includes('company-create') || permissions.includes('super-admin') ? (
                                        <Link
                                            href={`${base_url}/admin/companies/create`}
                                            method="get"
                        ...
                        <div className="datatables">
                            <DataTable
                                ...
                                columns={[
                                    {
                                        accessor: "groupcompany",
                                        title: "Group Company",
                                        render: ({ groupcompany }) => (
                                            <div className="flex items-center w-max">
                                                <div>{groupcompany?.name}</div>
                                            </div>
                                        ),
                                    },
                                    {
                                    ...
                                    },
                                    {
                                        ...
                                        render: (result) => (
                                            <div className="flex items-center w-max mx-auto gap-2">
                                                {
                                                    permissions.includes('company-edit') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/companies/edit/`+result.id} method="get" className="btn btn-sm btn-outline-primary">
                                                            Edit
                                                        </Link>
                                                    ) : null
                                                }
                                                {
                                                    permissions.includes('company-delete') || permissions.includes('super-admin') ? (
                                                <Link href={`${base_url}/admin/companies/delete/`+result.id} method="get" className="btn btn-sm btn-outline-danger">
                                                    Delete
                                                </Link>
                                                    ) : null
                                                }
...
Index.layout = (page) => (
    <MainLayout children={page} title="HR || Company" />
);
