...
function Index() {
    ...
    function editData(result) {
        router.get("/admin/working_status/edit/" + result.id);
    }
    function deleteData(result) {
        router.get("/admin/working_status/delete/" + result.id);
    }
    function statusChanged(result) {
        router.get("/admin/working_status/status/" + result.id);
    }
    return (
        <>
            ...
            {
                permissions.includes('working-status-view') || permissions.includes('super-admin') ? (
                    <div className="panel mt-6">
                        <div className="flex md:items-center flex-col sm:flex-row justify-between mb-5 gap-5">
                            ...
                            <div className="flex md:items-center gap-5">
                                ...
                                {
                                    permissions.includes('working-status-create') || permissions.includes('super-admin') ? (
                                        <Link
                                            href={`${base_url}/admin/working_status/create`}
                                            method="get"
                                    ...
                                    },
                                    {
                                        ...
                                        render: (result) => (
                                            <div className="flex items-center w-max mx-auto gap-2">
                                                {
                                                    permissions.includes('working-status-edit') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/working_status/edit/`+result.id} method="get" className="btn btn-sm btn-outline-primary">
                                                            Edit
                                                        </Link>
                                                    ) : null
                                                }
                                                {
                                                    permissions.includes('working-status-delete') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/working_status/delete/`+result.id} method="get" className="btn btn-sm btn-outline-danger">
                                                            Delete
                                                        </Link>
                                                    ) : null
                                                }
...
Index.layout = (page) => (
    <MainLayout children={page} title="HR || Working Status" />
);
