...
function Index() {
    ...
    const [initialRecords2, setInitialRecords2] = useState(result, "username");
    ...
    useEffect(() => {
        setInitialRecords2(() => {
            return result.filter((item) => {
                return (
                    item.user?.username.toLowerCase().includes(search2.toLowerCase())
                );
            });
        });
    }, [search2]);
    ...
    function editGroupCompany(result) {
        router.get("/admin/arears/edit/" + result.id);
    }
    function deleteGroupCompany(result) {
        router.get("/admin/arears/delete/" + result.id);
    }
    function statusChanged(result) {
        router.get("/admin/arears/status/" + result.id);
    }
    return (
        <>
            ...
            {
                permissions.includes('department-view') || permissions.includes('super-admin') ? (
                    <div className="panel mt-6">
                        <div className="flex md:items-center flex-col sm:flex-row justify-between mb-5 gap-5">
                            ...
                            <div className="flex md:items-center gap-5">
                                ...
                                {
                                    permissions.includes('department-create') || permissions.includes('super-admin') ? (
                                        <Link
                                            href={`${base_url}/admin/arears/create`}
                                            method="get"
                                    ...
                                    },
                                    {
                                        ...
                                        render: (result) => (
                                            <div className="flex items-center w-max mx-auto gap-2">
                                                {
                                                    permissions.includes('department-edit') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/arears/edit/`+result.id} method="get" className="btn btn-sm btn-outline-primary">
                                                            Edit
                                                        </Link>
                                                    ) : null
                                                }
                                                {
                                                    permissions.includes('department-delete') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/arears/delete/`+result.id} method="get" className="btn btn-sm btn-outline-danger">
                                                            Delete
                                                        </Link>
                                                    ) : null
                                                }
...
Index.layout = (page) => (
    <MainLayout children={page} title="HR || Department" />
);
