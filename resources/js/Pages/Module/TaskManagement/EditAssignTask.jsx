...
function Add() {
    ...
    const selectedAssignedUser = result.users.map((user) => user.id);
    ...
    const handleSelectUser = (selectedAssignedUser) => {
        setValues((prevValues) => ({
            ...prevValues,
            selectedAssignedUser: selectedAssignedUser.map((user) => user.value),
        }));
    };
...
Add.layout = (page) => (
    <MainLayout children={page} title="HR || Edit Assign Task" />
);
