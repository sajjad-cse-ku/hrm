...
function Index() {
    ...
    function editData(result) {
        router.get("/admin/task-management/edit/" + result.id);
    }
    function deleteData(result) {
        router.get("/admin/task-management/delete/" + result.id);
    }
    function statusChanged(result) {
        router.get("/admin/task-management/status/" + result.id);
    }
    return (
        <>
                ...
                <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
                    ...
                    <div className="ltr:ml-auto rtl:mr-auto">
                        ...
                        />
                         ...
                         <Link
                            href={`${base_url}/admin/task-management/create`}
                            method="get"
                                ...
                                render: (result) => (
                                    <div className="flex items-center w-max mx-auto gap-2">
                                        ...
                                        {permissions.includes('shift-edit') || permissions.includes('super-admin') &&
                                            <Link href={`${base_url}/admin/task-management/edit/`+result.id} method="get" className="btn btn-sm btn-outline-primary">
                                                Edit
                                            </Link>
                                        }
                                        {permissions.includes('shift-delete') || permissions.includes('super-admin') &&
                                            <Link href={`${base_url}/admin/task-management/delete/`+result.id} method="get" className="btn btn-sm btn-outline-danger">
                                                Delete
                                            </Link>
                                        }
