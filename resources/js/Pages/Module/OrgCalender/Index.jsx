...
function Index() {
    ...
    useEffect(() => {
        setInitialRecords2(() => {
            return result.filter((item) => {
                const calendarYearAsString = item.calender_year.toString();
                return (
                    calendarYearAsString.toLowerCase().includes(search2.toLowerCase())
                );
            });
        });
    }, [search2]);
    ...
    function deleteData(result) {
        router.get("/admin/org_calender/delete/" + result.id);
    }
    function statusChanged(result) {
        router.get("/admin/org_calender/status/" + result.id);
    }
    return (
        <>
            ...
            {
                permissions.includes('org-calendar-view') || permissions.includes('super-admin') ? (
                    <div className="panel mt-6">
                        <div className="flex md:items-center flex-col sm:flex-row justify-between mb-5 gap-5">
                            ...
                            <div className="flex md:items-center gap-5">
                                ...
                                {
                                    permissions.includes('org-calendar-create') || permissions.includes('super-admin') ? (
                                        <Link
                                            href={`${base_url}/admin/org_calender/create`}
                                            method="get"
                                    ...
                                    },
                                    {
                                        ...
                                        render: (result) => (
                                            <div className="flex items-center w-max mx-auto gap-2">
                                                {
                                                    permissions.includes('org-calendar-edit') || permissions.includes('super-admin') ? (
                                                    <Link href={`${base_url}/admin/org_calender/edit/`+result.id} method="get" className="btn btn-sm btn-outline-primary">
                                                        Edit
                                                    </Link>
                                                    ) : null
                                                }
                                                ...
                                                {
                                                    permissions.includes('org-calendar-delete') || permissions.includes('super-admin') ? (
                                                    <Link href={`${base_url}/admin/org_calender/delete/`+result.id} method="get" className="btn btn-sm btn-outline-danger">
                                                        Delete
                                                    </Link>
                                                    ) : null
                                                }
...
Index.layout = (page) => (
    <MainLayout children={page} title="HR || Org Calender" />
);
