...
function Index() {
    ...
    function editData(result) {
        router.get("/admin/religions/edit/" + result.id);
    }
    function deleteData(result) {
        router.get("/admin/religions/delete/" + result.id);
    }
    function statusChanged(result) {
        router.get("/admin/religions/status/" + result.id);
    }
    return (
        <>
            ...
            {
                permissions.includes('religion-view') || permissions.includes('super-admin') ? (
                    <div className="panel mt-6">
                        <div className="flex md:items-center flex-col sm:flex-row justify-between mb-5 gap-5">
                            ...
                            <div className="flex md:items-center gap-5">
                                ...
                                {
                                    permissions.includes('religion-create') || permissions.includes('super-admin') ? (
                                        <Link
                                            href={`${base_url}/admin/religions/create`}
                                            method="get"
                                    ...
                                    },
                                    {
                                        ...
                                        render: (result) => (
                                            <div className="flex items-center w-max mx-auto gap-2">
                                                {
                                                    permissions.includes('religion-edit') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/religions/edit/`+result.id} method="get" className="btn btn-sm btn-outline-primary">
                                                            Edit
                                                        </Link>
                                                    ) : null
                                                }
                                                {
                                                    permissions.includes('religion-delete') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/religions/delete/`+result.id} method="get" className="btn btn-sm btn-outline-danger">
                                                            Delete
                                                        </Link>
                                                    ) : null
                                                }
...
Index.layout = (page) => (
    <MainLayout children={page} title="HR || Religions" />
);
