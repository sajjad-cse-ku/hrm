...
function Index() {
    const { base_url, flash , user_id ,user, results, permissions,designationName } = usePage().props;
    const PAGE_SIZES = [10, 20, 30, 50, 100];
    const [page2, setPage2] = useState(1);
    const [pageSize2, setPageSize2] = useState(PAGE_SIZES[0]);
    const [initialRecords2, setInitialRecords2] = useState(results, "name");
    const [recordsData2, setRecordsData2] = useState(initialRecords2);

    const [search2, setSearch2] = useState("");
    const [sortStatus2, setSortStatus2] = useState({
        columnAccessor: "posting_start_date",
        direction: "asc",
    });
    ...
    useEffect(() => {
        setInitialRecords2(() => {
            return results.filter((item) => {
                return (
                    item.effective_date.toLowerCase().includes(search2.toLowerCase())
                );
            });
        });
    }, [search2]);
    ...
    //     router.get("/admin/employee_posting/edit/" + result.id);
    // }
    // function deleteData(result) {
    //     router.get("/admin/employee_posting/delete/" + result.id);
    // }
    function statusChanged(result) {
        router.get("/admin/emp_promotion/status/" + result.id);
    }
    return (
        <>
                                ...
                                <div className="mt-6">
                                    <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
                                        ...
                                        <div className="ltr:ml-auto rtl:mr-auto">
                                            {
                                                permissions.includes('emp-promotion-create') || permissions.includes('super-admin') ? (
                                                    <Link
                                                        href={`${base_url}/admin/emp_promotion/create/${user?.id}`}
                                                        method="get"
                                                ...
                                                },
                                                {
                                                    ...
                                                    render: (result) => (
                                                        <div className="flex items-center w-max mx-auto gap-2">
                                                            {
                                                                permissions.includes('emp-promotion-edit') || permissions.includes('super-admin') ? (
                                                                    <Link href={`${base_url}/admin/emp_promotion/edit/`+result.id} method="get" className="btn btn-sm btn-outline-primary">
                                                                        Edit
                                                                    </Link>
                                                                ) : null
                                                            }
                                                            {
                                                                permissions.includes('emp-promotion-delete') || permissions.includes('super-admin') ? (
                                                                    <Link href={`${base_url}/admin/emp_promotion/delete/`+result.id} method="get" className="btn btn-sm btn-outline-danger">
                                                                        Delete
                                                                    </Link>
                                                                ) : null
                                                            }
...
Index.layout = (page) => (
    <MainLayout children={page} title="HR || Promotions" />
);
