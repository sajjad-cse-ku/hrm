...
function Index() {
    ...
    function editData(result) {
        router.get("/admin/title/edit/" + result.id);
    }
    function deleteData(result) {
        router.get("/admin/title/delete/" + result.id);
    }
    function statusChanged(result) {
        router.get("/admin/title/status/" + result.id);
    }
    return (
        <>
            ...
            {
                permissions.includes('title-view') || permissions.includes('super-admin') ? (
                    <div className="panel mt-6">
                        <div className="flex md:items-center flex-col sm:flex-row justify-between mb-5 gap-5">
                            ...
                            <div className="flex md:items-center gap-5">
                                ...
                                {
                                    permissions.includes('title-create') || permissions.includes('super-admin') ? (
                                        <Link
                                            href={`${base_url}/admin/title/create`}
                                            method="get"
                                    ...
                                    },
                                    {
                                        ...
                                        render: (result) => (
                                            <div className="flex items-center w-max mx-auto gap-2">
                                                {
                                                    permissions.includes('title-edit') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/title/edit/`+result.id} method="get" className="btn btn-sm btn-outline-primary">
                                                            Edit
                                                        </Link>
                                                    ) : null
                                                }
                                                {
                                                    permissions.includes('title-delete') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/title/delete/`+result.id} method="get" className="btn btn-sm btn-outline-danger">
                                                            Delete
                                                        </Link>
                                                    ) : null
                                                }
