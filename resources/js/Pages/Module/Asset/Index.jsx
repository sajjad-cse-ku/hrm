...
function Index() {
    const { base_url, flash, result, permissions, assetType } = usePage().props;
    // console.log(result);
    const PAGE_SIZES = [10, 20, 30, 50, 100];
    const [page2, setPage2] = useState(1);
    const [pageSize2, setPageSize2] = useState(PAGE_SIZES[0]);
    const [initialRecords2, setInitialRecords2] = useState(result, "product_name");
    ...
    useEffect(() => {
        setInitialRecords2(() => {
            return result.filter((item) => {
                return (
                    item.product_name.toLowerCase().includes(search2.toLowerCase())
                );
            });
        });
    }, [search2]);
    ...
    function statusChanged(result) {
        router.get("/admin/asset/status/" + result.id);
    }
                    ...
                    <div className="panel mt-6">
                        <div className="flex md:items-center flex-col sm:flex-row justify-between mb-5 gap-5">
                            ...
                            <div className="flex md:items-center gap-5">
                                ...
                                {/*    permissions.includes('super-admin') ? (*/}
                                        <Link
                                            href={`${base_url}/admin/asset/create`}
                                            method="get"
                                    ...
                                    },
                                    {
                                        ...
                                        render: (result) => (
                                            <div className="flex items-center w-max mx-auto gap-2">
                                                {/*{*/}
                                                {/*    permissions.includes('super-admin') ? (*/}
                                                        <Link href={`${base_url}/admin/asset/edit/`+result.id} method="get" className="btn btn-sm btn-outline-primary">
                                                            Edit
                                                        </Link>
                                                {/*    ) : null*/}
                                                {/*}*/}
                                                {/*{*/}
                                                {/*    permissions.includes('super-admin') ? (*/}
                                                        <Link href={`${base_url}/admin/asset/delete/`+result.id} method="get" className="btn btn-sm btn-outline-danger">
                                                            Delete
                                                        </Link>
                                                {/*    ) : null*/}
                                                {/*}*/}
...
Index.layout = (page) => (
    <MainLayout children={page} title="HR || Asset List" />
);
