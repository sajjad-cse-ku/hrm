...
function Index() {
    ...
    useEffect(() => {
        setInitialRecords2(() => {
            return result.filter((item) => {
                ...
                const to_date = item?.to_date;
                const title = item?.title;

                return (

                    (from_date && from_date.includes(search)) || // Check for joining date match
                    (to_date && to_date.includes(search)) || // Check for joining date match
    ...
    function editData(result) {
        router.get("/admin/public_holiday/edit/" + result.id);
    }
    function deleteData(result) {
        router.get("/admin/public_holiday/delete/" + result.id);
    }
    function statusChanged(result) {
        router.get("/admin/public_holiday/status/" + result.id);
    }
    return (
        <>
            ...
            {
                permissions.includes('public-holiday-view') || permissions.includes('super-admin') ? (
                    <div className="panel mt-6">
                        <div className="flex md:items-center flex-col sm:flex-row justify-between mb-5 gap-5">
                            ...
                            <div className="flex md:items-center gap-5">
                                ...
                                {
                                    permissions.includes('public-holiday-create') || permissions.includes('super-admin') ? (
                                        <Link
                                            href={`${base_url}/admin/public_holiday/create`}
                                            method="get"
                                    ...
                                    },
                                    {
                                        ...
                                        render: (result) => (
                                            <div className="flex items-center w-max mx-auto gap-2">
                                                {
                                                    permissions.includes('public-holiday-edit') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/public_holiday/edit/`+result.id} method="get" className="btn btn-sm btn-outline-primary">
                                                            Edit
                                                        </Link>
                                                    ) : null
                                                }
                                                {
                                                    permissions.includes('public-holiday-delete') || permissions.includes('super-admin') ? (
                                                        <Link href={`${base_url}/admin/public_holiday/delete/`+result.id} method="get" className="btn btn-sm btn-outline-danger">
                                                            Delete
                                                        </Link>
                                                    ) : null
                                                }
