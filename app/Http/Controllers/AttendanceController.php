...
class AttendanceController extends Controller
{
    public function attendance(Request $request)
    {
        ...
        $general_time = Shift::where('id', SiteSettings()->general_shift)->first();

        if(count($isData) > 0){
            foreach ($companies as $company) {

                $public_holiday = $this->getCompnayBasePublicHoliDays($currentOfMonth,$endOfMonth,$currentDate,$company->id);
                $userAttendanceData = $this->userAttendanceData($company,$currentDate,$currentOfMonth,$endOfMonth);
//                dd($userAttendanceData[10]);

                // New loop Add for testing
                foreach ($userAttendanceData as $key=>$attendanceData) {

                    $type = null ;
                    $user = $attendanceData['user'];

                    $leaves = $attendanceData['leaves']->where('status','A');
                    ...
                    $days_name = RosterAssign::where('user_id', $user->id)->pluck('name');


                    $days_name_array = $days_name->toArray();


                    $req_date = Carbon::createFromFormat('Y-m-d',$request->set_date)->format('D');

                    $currentDayName = $req_date ?? Carbon::now()->format('D');

                    if(isset($days_name_array)){
                        if (in_array(strtolower($currentDayName), $days_name_array)) {
                            $offday_flag  = 1;
                        }else {
                    ...
                    $leave_id = $leaves->isNotEmpty() ? $leaves->first()->leave_id : 0;
    ...
    public function attendanceDataDbInsert($user,$enrtyExitData,$userWithRoster,$getUserLateData,$leave_flag,$leave_id,$public_holiday,$offday_flag,$offday_present,$created_by,$currentDate){
        ...
        try {
            ...
            $existingData = Attendance::where('user_id', $user->id)
                ->where('attend_date', $currentDate)
                ->first();

            if (
                ($existingData['attend_status'] ?? null) === "R" ||
                ($existingData['manual_update'] ?? null) === 1
            ) {
                DB::commit();
                return false;
            }
                ...
                $save = Attendance::updateOrCreate(
                    ...
                    [
                        ...
                        'user_id' => $user->id,
                        'department_id' => $user->professionaldata->department_id ?? 1,
                        'device_id' => 1,
                        ...
                        'exit_date' => $enrtyExitData['entry_date'] === $enrtyExitData['exit_date'] ? null : $enrtyExitData['exit_date'],
                        'exit_time' => $enrtyExitData['entry_time'] === $enrtyExitData['exit_time'] ? null : $enrtyExitData['exit_time'],
    ...
    public function userAttendanceData($company,$currentDate,$currentOfMonth,$endOfMonth){
        ...
        foreach ($company->user as $user) {
            ...
            $l_events = $user->leaveapplication()->where(function ($query) use ($currentOfMonth, $endOfMonth) {
                $query->where('from_date', '<=', $endOfMonth)
                    ->where('to_date', '>=', $currentOfMonth);
            })->get();

            $leaves = $l_events->filter(function ($event) use ($currentDate) {
//                dd($event->from_date,$event->to_date);
                return $event->from_date <= $currentDate && $event->to_date >= $currentDate;
            });
    ...
    public function enrtyExitData($attendanceData,$shift_id,$type,$leaves){
        ...
        if($type == "hr_manual"){
            ...
            $min_attendance_datetime = Carbon::parse("{$attendanceDate} {$inTime}");
            $exit_date =$attendanceData["exit_date"];
            $out_time = $attendanceData["out_time"];

            $max_attendance_datetime = Carbon::parse("{$exit_date} {$out_time}");
        ...
        $night_duty = in_array($shift_id,
         [1, 2]) ? 1 : 0;

//        $night_duty = in_array($shift_id,SiteSettings()->night_shift) ? $shift_id : SiteSettings()->general_shift;
            ...
            if (
                ...
                $save = Attendance::updateOrCreate(
                    ...
                    [
                        ...
                        'attend_status'=> "O" ,
                        'night_duty'=> 0 ,
                        'late_flag' =>$existingData->manual_late_allow == 1 ? 0 : $getUserLateData['late_flag'] ?? 0,
                        'late_allow'=> 0 ,
                        'late_minute'=> 0 ,
                        'over_time'=> 0 ,
                        'overtime_hour'=> 0 ,
    ...
    public function todayAttendance(){
        ...
        $results = User::with(['attendance' => function ($query) use ($currentDate) {
            $query->with(['shift'=>function($shiftQuery){
                ...
                ->select('id', 'user_id', 'leave_id', 'attend_date', 'entry_time', 'exit_time', 'holiday_flag', 'leave_id', 'shift_id','offday_flag', 'late_flag', 'attendance_datetime','manual_update_remarks','attend_status','home_attendance');
        }])->where('is_show',1)
            ->where('status',1)
    ...
    public function manualAttendanceUpdate(Request $request){
        ...
        if ($user !== null && $user->leaveapplication !== null) {
            $l_events = $user->leaveapplication()->where(function ($query) use ($currentOfMonth, $endOfMonth) {
                $query->where('from_date', '<=', $endOfMonth)
                      ->where('to_date', '>=', $currentOfMonth);
            })->get();
        ...
        return to_route('admin.dashboard')->with('success','Attendance Successfully Added');
    }
    ...
    public function manualCommentUpdate(Request $request){
        ...
        if($attendance){
            ...
            return to_route('admin.dashboard')->with('success','comment done');
        }
    ...
    public function homeOfficeAttendance(){
        ...
        $attend_status = $minCarbon != null ? 'R' :
            ($shift_id == 1 ? 'O' :
                (isset($leaves->id )? 'L' : 'A'));
        $night_duty = in_array($shift_id, [5,9,11]) ? 1 : 0;
    ...
    public function myAttendanceReport($id){
        if ($id == Auth::id()) {
            $currentMonth = Carbon::now('Asia/Dhaka');
            $general_time = Shift::where('id', 2)->first();
    ...
    public function homeOffice(){
        $break_times = BreakTime::where('user_id',Auth::id())->where('start_date',now('Asia/Dhaka')->format('Y-m-d'))->get();

        $button = Attendance::where('user_id', Auth::id())
    ...
    public function homeOfficeAttendanceSave(Request $request){
        ...
        $data = BreakTime::where('user_id', Auth::id())->where('start_date', now('Asia/Dhaka')->format('Y-m-d'))->latest()->first();

        if ($data && $data->end_date) {
            // If there's an existing record with an end_date, insert a new record
            $newEvent = new BreakTime();
            $newEvent->user_id = Auth::id();
            $newEvent->start_date = now('Asia/Dhaka')->toDateString();
            $newEvent->start_time = now('Asia/Dhaka')->toTimeString();
            $newEvent->save();

            return response()->json(BreakTime::where('user_id', Auth::id())->where('start_date', now('Asia/Dhaka')->format('Y-m-d'))->latest()->get());
        } elseif ($data) {
            $request->validate([
                'comment' => 'required',
            ]);
            // If there's an existing record without an end_date, update it
            $data->end_date = now('Asia/Dhaka')->toDateString();
            $data->end_time = now('Asia/Dhaka')->toTimeString();
            $formattedDuration  = now('Asia/Dhaka')->diff($data->start_time);
